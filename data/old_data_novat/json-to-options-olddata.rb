...
json_file = File.open('data-olddata.json', 'r')
orgs = JSON.parse(json_file.read)
json_file.close

current_orgs_options = File.open("options-olddata.html", "wb")
all_orgs_options = File.open("options-all-alldata.html", "wb")
...
closed_orgs.sort_by {|org| org['current_name'] }.each do |org|
  ...
  html += "  <option value=\"#{org['key']}\" #{data_abbeviations} #{data_other_names}>#{org['current_name'].gsub('&', '&amp;')} - #{org['category'].gsub('&', '&amp;')}</option>\n"
end
html += "</optgroup>\n"
